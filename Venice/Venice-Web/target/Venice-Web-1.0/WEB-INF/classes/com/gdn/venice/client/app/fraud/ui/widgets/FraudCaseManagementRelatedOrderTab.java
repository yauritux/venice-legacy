...
public class FraudCaseManagementRelatedOrderTab extends Tab {
	ListGrid relatedOrderListGrid = null;
	ListGrid relatedOrderListGridPrint = null;
	
	public FraudCaseManagementRelatedOrderTab(String title, DataSource relatedOrderData, DataSource relatedOrderDataPrint) {
		super(title);		        
...
//		relatedOrderListGrid.getField(DataNameTokens.VENORDER_ORDERDATE).setDateFormatter(DateDisplayFormat.TOEUROPEANSHORTDATETIME);
		...
		ToolStripButton removeFromRelatedOrder = new ToolStripButton();
		removeFromRelatedOrder.setIcon("[SKIN]/icons/delete.png");  
		removeFromRelatedOrder.setTooltip("Remove from Related Orders");
		removeFromRelatedOrder.setTitle("Remove");
		
		removeFromRelatedOrder.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				SC.ask("Are you sure you want to delete this data?", new BooleanCallback() {					
					@Override
					public void execute(Boolean value) {
						if(value != null && value){
							relatedOrderListGrid.removeSelectedData();
							SC.say("Data Removed");			
						}
					}
				});
...
//		printRelatedOrder.setTitle("Print");
		...
		relatedOrderToolStrip.addButton(removeFromRelatedOrder);
//		relatedOrderToolStrip.addSeparator();
//		relatedOrderToolStrip.addButton(printRelatedOrder);
		...
		relatedOrderListGridPrint.getField(DataNameTokens.VENCUSTOMER_VENPARTY_FULLORLEGALNAME).setWidth("150");
