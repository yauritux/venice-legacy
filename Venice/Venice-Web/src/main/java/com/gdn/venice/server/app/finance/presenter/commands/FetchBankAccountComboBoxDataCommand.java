...
public class FetchBankAccountComboBoxDataCommand implements RafRpcCommand{
	...
	public String execute() {
		...
		try{
			...
			String query = "select o from FinAccount o where o.finAccountCategory.accountCategoryDesc='Kas dan Bank' order by o.accountDesc asc";
			
			finAccountList = sessionHome.queryByRange(query, 0, 100);
			
			for(int i=0; i<finAccountList.size();i++){
				FinAccount finAccount = finAccountList.get(i);
				map.put("data"+finAccount.getAccountId().toString(), finAccount.getAccountDesc());							
			}
