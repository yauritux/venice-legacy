...
public class FraudCaseViewerLayout extends HLayout {

	public FraudCaseViewerLayout(
		...
		fraudCaseManagementTabSet.addTab(new FraudCaseViewerFraudManagementTab("Fraud Management", riskScoreData, relatedOrderData, fraudManagementData, totalRiskScoreData, actionLogData, historyLogData, attachmentData, caseId));
		...
		fraudCaseManagementTabSet.addTabSelectedHandler(new TabSelectedHandler() {
			...
			public void onTabSelected(TabSelectedEvent event) {
				...
				} else if (event.getTab() instanceof FraudCaseViewerFraudManagementTab){
					((FraudCaseViewerFraudManagementTab) event.getTab()).getFraudCaseManagementForm().fetchData();
					((FraudCaseViewerFraudManagementTab) event.getTab()).getFraudCaseManagementTotalRiskForm().fetchData();
