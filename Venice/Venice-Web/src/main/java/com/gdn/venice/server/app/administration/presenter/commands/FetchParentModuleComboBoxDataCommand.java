...
/**
 * Fetch Command for Parent Module
 * 
 * @author Roland
 */

public class FetchParentModuleComboBoxDataCommand implements RafRpcCommand{
	...
	public String execute() {
		...
		try{
			...
			module = sessionHome.queryByRange(query, 0, 0);
			
			for(int i=0; i<module.size();i++){
				RafApplicationObject list = module.get(i);
				map.put("data"+Util.isNull(list.getApplicationObjectId(), "").toString(), Util.isNull(list.getApplicationObjectCanonicalName(),"").toString());							
			}
