...
public class KlikPayInst_CC_ReportImportServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (isMultipart) { // import
			...
			_log.debug("-----coba prinnnnnnnnnnnnnnnnnnttt");
			...
			try {						
				 ...
				 */
				String uniqueId = ((BCA_CC_Record)result.get(0)).getTransDate().toString().replace(".", "");			
				if (!FinanceImportServletHelper.isFileUnique(uniqueId, VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAYINST_CC)) {
					String errMsg = FinanceImportServletConstants.EXCEPTION_TEXT_FILE_NOT_UNIQUE;
					notificationText = notificationText.replaceFirst("REPLACE", errMsg);
					response.getOutputStream().println(notificationText);
					return;
				}
				_log.debug("-----coba prinnnnnnnnnnnnnnnnnnttt2");
				...
				finArFundsInReport = FinanceImportServletHelper
						.createFundsInReportRecord(VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAYINST_CC, fileNameAndPath.substring(fileNameAndPath.lastIndexOf('/') + 1), filePath, uniqueId);
				...
				for (BCA_CC_Record element : results) {
					...
					VenOrder venOrder = FinanceImportServletHelper.getPaymentRelatedOrder(bcaCcRecord.getAuthCd(), new Double(bcaCcRecord.getGrossAmt()), VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAYINST_CC);
...
//					}
					...
					if(venOrder!=null){
						 reconRecord = FinanceImportServletHelper
							.getReconciliationRecord(em, VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAYINST_CC, bcaCcRecord.getAuthCd(), new BigDecimal(bcaCcRecord.getGrossAmt()));
					}
						 if (reconRecord == null) {
					...
					VenOrderPayment venOrderPayment = FinanceImportServletHelper.getRelatedPaymentRecord(bcaCcRecord.getAuthCd(),new Double(bcaCcRecord.getGrossAmt()),  VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAYINST_CC);
