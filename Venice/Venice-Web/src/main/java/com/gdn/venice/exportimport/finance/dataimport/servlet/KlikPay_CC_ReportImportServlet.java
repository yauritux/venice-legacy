 ...
 * 
 */
public class KlikPay_CC_ReportImportServlet extends HttpServlet {
	 ...
	 */
	public KlikPay_CC_ReportImportServlet() {
		super();
		Log4jLoggerFactory loggerFactory = new Log4jLoggerFactory();
		_log = loggerFactory.getLog4JLogger("com.gdn.venice.finance.dataimport.servlet.klikPay_CC_ReportImportServlet");
	}
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		_log.debug("KlikPay_CC_ReportImportServlet:hello");
		...
		if (isMultipart) { // import
			...
			// Upload the file
			String fileNameAndPath = FinanceImportServletHelper.upload(request, filePath, "-klikPay_CC_Report.xls");
			...
			try {						
				 ...
				 */
				String uniqueId = ((BCA_CC_Record)result.get(0)).getTransDate().toString().replace(".", "").replace("E", "0");			
				if (!FinanceImportServletHelper.isFileUnique(uniqueId, VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_CC)) {
				 ...
				 */
				 finArFundsInReport = FinanceImportServletHelper
						.createFundsInReportRecord(VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_CC, fileNameAndPath.substring(fileNameAndPath.lastIndexOf('/') + 1), filePath, uniqueId);
				...
				_log.debug("-----End cek duplikasi row report");
				ArrayList<FinArFundsInReconRecord> newFinArFundsInReconRecord = new ArrayList<FinArFundsInReconRecord>();
				int newRowNumber=0;
				for (BCA_CC_Record element : results) {
					BCA_CC_Record bcaCcRecord = element;
					...
					VenOrder venOrder = FinanceImportServletHelper.getPaymentRelatedOrder(bcaCcRecord.getAuthCd(),new Double(bcaCcRecord.getGrossAmt()), VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_CC);
...
//					}
					...
					if(venOrder!=null){
						 reconRecord = FinanceImportServletHelper
							.getReconciliationRecord(em, VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_CC, bcaCcRecord.getAuthCd(), new BigDecimal(bcaCcRecord.getGrossAmt()));
					}
						 if (reconRecord == null) {
						reconRecord = new FinArFundsInReconRecord();
					}						 
					Date tgl=sdf.parse(bcaCcRecord.getTransDate().replace(".", "").replace("E", "0")+" "+times);
					reconRecord.setProviderReportPaymentDate(new java.sql.Timestamp(tgl.getTime()));
					if(FinanceImportServletHelper.cekContinueProsesFundIn(em, VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_BCA_CC, bcaCcRecord.getAuthCd(),reconRecord.getProviderReportPaymentDate()+"", new BigDecimal(bcaCcRecord.getGrossAmt()))){
					...
					if(venOrder!=null &&  reconRecord.getVenOrderPayment() != null){
						...
						reconRecord.setNomorReff(bcaCcRecord.getAuthCd());
					}
					
					reconRecord.setPaymentConfirmationNumber(bcaCcRecord.getAuthCd());
					...
					reconRecord.setProviderReportPaymentId(bcaCcRecord.getAuthCd());
					reconRecord.setReconcilliationRecordTimestamp(new java.sql.Timestamp(System.currentTimeMillis()));					

					VenOrderPayment venOrderPayment = FinanceImportServletHelper.getRelatedPaymentRecord(bcaCcRecord.getAuthCd(), new Double(bcaCcRecord.getGrossAmt()), VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_CC);

					/*
					 * Check if the order is recognized in venice database, it must have payment record.
					 */
					if(venOrder!=null && venOrderPayment == null){
						String errMsg = FinanceImportServletConstants.EXCEPTION_TEXT_PAYMENT_NOT_FOUND + bcaCcRecord.getAuthCd();
						_log.error(errMsg);
						if(!notificationText.contains("REPLACE")){
							notificationText = notificationText.replaceFirst("VENICE:", "VENICE:" + bcaCcRecord.getAuthCd() + " ,");
						}else{
				...
				/*	if (reconRecord.getRemainingBalanceAmount().compareTo(new BigDecimal(0)) == 0) {
					...
					BigDecimal remainingBalanceAmount =reconRecord.getRemainingBalanceAmount().abs().compareTo(VeniceConstants.TRACEHOLD_RECEIVED) <= 0 ? new BigDecimal(0):reconRecord.getRemainingBalanceAmount();
