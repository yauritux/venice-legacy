...
/**
 * This Datasource-style presenter servlet command use for fetching Party Addresss data
 *
 * <p>
 ...
 * <b>since:</b> 2011
 */
public class FetchAddressDataCommand implements RafDsCommand {
	...
	/**
	 * Basic constructor for Fetch Party Address data
	 * @param rafDsRequest is the request parameters passed to the command 
	 */
	public FetchAddressDataCommand(RafDsRequest rafDsRequest) {
	...
	public RafDsResponse execute() {
		...
		List<HashMap<String, String>> venPartyAddressDataList= new ArrayList<HashMap<String, String>>();
		...
		try{
			...
			/*
			 * This is the list that will be used to fetch the party address
			 */
			...
			if (criteria == null) {
				if(request.getParams() != null && request.getParams().get(DataNameTokens.VENPARTYADDRESS_VENPARTY_PARTYID) != null){
					String query = "select o from VenPartyAddress o where o.venParty.partyId = " + request.getParams().get(DataNameTokens.VENPARTYADDRESS_VENPARTY_PARTYID);	
			...
			for(VenPartyAddress partyAddress:venPartyAddressList){
				...
				map.put(DataNameTokens.VENPARTYADDRESS_VENADDRESS_VENCITY_CITYID, partyAddress.getVenAddress().getVenCity().getCityId() !=null?partyAddress.getVenAddress().getVenCity().getCityId()+"":"");
				...
				map.put(DataNameTokens.VENPARTYADDRESS_VENADDRESS_VENCOUNTRY_COUNTRYNAME, partyAddress.getVenAddress().getVenCountry().getCountryName()!=null?partyAddress.getVenAddress().getVenCountry().getCountryName():"");
				...
				venPartyAddressDataList.add(map);
			}
		...
		rafDsResponse.setData(venPartyAddressDataList);
