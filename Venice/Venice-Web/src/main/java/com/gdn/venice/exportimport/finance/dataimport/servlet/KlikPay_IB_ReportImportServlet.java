	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (isMultipart) { // import
			...
			try {
				...
				if(!FinanceImportServletHelper.isFileUnique(uniqueId, VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_IB)){
				 ...
				 */
				finArFundsInReport = FinanceImportServletHelper.createFundsInReportRecord(VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_IB, fileNameAndPath.substring(fileNameAndPath.lastIndexOf('/')+1), filePath, uniqueId);
				...
				for (BCA_VA_IB_Record element : recordItems) {
					...
					VenOrder venOrder = FinanceImportServletHelper.getPaymentRelatedOrder(bcaIBRecord.getAccountNumber(), bcaIBRecord.getPaymentAmount(),VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_IB);
...
//					}
					...
					if(venOrder!=null){
						 reconRecord = FinanceImportServletHelper
							.getReconciliationRecord(em, VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_IB, bcaIBRecord.getAccountNumber(), new BigDecimal(bcaIBRecord.getPaymentAmount()));
					}
					if (reconRecord == null) {
						reconRecord = new FinArFundsInReconRecord();
					}
					if(FinanceImportServletHelper.cekContinueProsesFundIn(em, VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_IB, bcaIBRecord.getAccountNumber(),"",new BigDecimal(bcaIBRecord.getPaymentAmount()))){
					...
					VenOrderPayment venOrderPayment = FinanceImportServletHelper.getRelatedPaymentRecord(bcaIBRecord.getAccountNumber(),new Double(bcaIBRecord.getPaymentAmount()), VeniceConstants.FIN_AR_FUNDS_IN_REPORT_TYPE_KLIKPAY_IB);
